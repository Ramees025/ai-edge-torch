...
from . import export_utils as utils
...
def _build_flat_inputs(
  ...
  for node, arg in zip(placeholder_nodes, export_flat_args):
    ...
    shape = tuple(
        utils.IR_DYNAMIC if utils.is_torch_dynamic(s) else s
        for s in tensor_meta.shape
    )
    ir_inputs.append(
        ir.RankedTensorType.get(
            shape,
            utils.torch_dtype_to_ir_element_type(ctx, tensor_meta.dtype),
        )
    )
...
def exported_program_to_mlir(
  ...
  with utils.create_ir_context() as context, ir.Location.unknown():
    ...
    with ir.InsertionPoint(module.body):
      ...
      def _main(*args):
        interpreter.run(*args, enable_io_processing=False)
        num_mutations = len(exported_program.graph_signature.buffers_to_mutate)
        return interpreter.outputs[num_mutations:]

    passes.cse(module)
    passes.build_stablehlo_composite(module)

    # Inline by default for func.call ops from Jax bridged lowerings.
    passes.inline(module)
    # Required to normalize debuginfo of inlined Jax bridged lowerings ops.
